-- ==============================================================================
-- 🚨 MIGRACIÓN DEFINITIVA: CORRECCIÓN CRÍTICA DE fn_crear_pedido_web
-- La Parada del Sabor — 29 Ago 2026
-- ==============================================================================

-- 1. Asegurar columnas de delivery, origen y estado en ventas
...
ALTER TABLE public.ventas ADD COLUMN IF NOT EXISTS origen_pedido VARCHAR(50) DEFAULT 'web';
ALTER TABLE public.clientes ADD COLUMN IF NOT EXISTS actualizado_el TIMESTAMPTZ DEFAULT NOW();

-- 2. Asegurar que el CHECK de estado en ventas acepte 'pendiente' y 'lista'
...
-- 3. Función RPC Definitiva y Segura (Soporta UUIDs en ventas_items, extras y delivery)
...
BEGIN
    ...
    v_delivery_zona_id := trim(p_payload->>'delivery_zona_id');
    ...
    IF v_telefono IS NULL OR length(v_telefono_limpio) < 7 THEN
    ...
    -- 2. Procesar tarifa de Delivery si aplica
    IF v_tipo_entrega = 'delivery' AND v_delivery_zona_id IS NOT NULL AND v_delivery_zona_id <> '' THEN
        BEGIN
            SELECT nombre, coalesce(precio_usd, 0)
            INTO v_delivery_zona_nombre, v_delivery_monto_usd
            FROM public.zonas_delivery
            WHERE id::TEXT = v_delivery_zona_id OR nombre ILIKE '%' || v_delivery_zona_id || '%'
            LIMIT 1;
        EXCEPTION WHEN OTHERS THEN
            v_delivery_zona_nombre := 'Delivery';
            v_delivery_monto_usd := 0;
        END;

        IF v_delivery_monto_usd IS NOT NULL AND v_delivery_monto_usd > 0 THEN
...
-- Permisos públicos para que la web de clientes pueda ejecutarlo sin login
