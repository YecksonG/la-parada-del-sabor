...
-- ==============================================================================
-- 3. TRIGGERS DE INVENTARIO Y RECONCILIACIÓN DE CANCELACIONES (BLINDADOS)
-- ==============================================================================

-- A) Descontar receta al insertar item de venta (solo cuando confirmada)
...
-- B) Descontar extra vendido (exclusividad mutua Método A vs B)
...
-- C) Confirmar pedido web (descuenta inventario al pasar de 'pendiente' a confirmada)
...
BEGIN
    IF OLD.estado = 'pendiente' AND NEW.estado IN ('preparando', 'lista', 'completada') THEN
        ...
        -- Descontar extras Método A (excluyendo extras con desglose en extras_ingredientes)
        UPDATE public.insumos i
...
-- D) Reconciliar stock ante cambio de estado en la venta (cancelaciones y reactivaciones)
...
BEGIN
    ...
    IF OLD.estado IN ('preparando', 'lista', 'completada') AND NEW.estado = 'cancelada' THEN
        ...
        -- Restaurar extras Método A (solo si NO tiene registros en extras_ingredientes)
        UPDATE public.insumos i
        ...
        -- Restaurar extras Método B (multi-insumo)
        UPDATE public.insumos i
        SET stock_actual   = i.stock_actual + sub.total_devuelto,
            actualizado_el = NOW()
        FROM (
    ...
    -- 2. Reactivación: Si pasa de 'cancelada' a confirmada, volver a descontar
